...
const string & table_name, const string & attribute_name) throw(Error)
{
    ...
    string preIndex_name;
    if(table->isIndex(attribute_name, preIndex_name) == true)
    {
        string err_info = "[CatalogManager::create_index] index '" + preIndex_name + "' already exists on the attribute '" + attribute_name + "'";
        Error err(err_info);
        throw err;
    }
