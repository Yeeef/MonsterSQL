...
int BPTree::insert(int nodeID, BPTreeKey& entry) {
    ...
    }else if (res == BPInsert)
    {
        ...
        if((res != BPRepeat ) && node->isoverflow())
        {
            ...
            node->split(node->getNextID(), entry, newID);
...
int BPTree::remove(int nodeID, BPTreeKey &entry, int siblingID, bool isLeftSib, ptr& retPointer, BPTreeKey & parentKey ) {
    ...
    if (res == BPDelete)
    {
        ...
        {
            //把兄弟结点new出来
            sibling = new BPTreeNode(fileName.c_str(), siblingID, keyLength);
...
int BPTree::findKey(BPTreeKey &entry) {
    ...
    }else
    {
        ...
        while(true)
        {
            ...
            if(node->isleaf() && node->getEntry(pos) == entry)
            {
                entry.setPointer(node->getEntry(pos).getPointer());
                ret = BPNormal;
                break;
            } else if(node->isleaf() && node->getEntry(pos) != entry )
            {
